...
*/
var lerp$1 = (start, end, factor) => factor === 1 ? end : factor === 0 ? start : start + (end - start) * factor;
/**
...
var composeComplexValue = (tween, progress, precision) => {
	...
	for (let j = 0, l = tn.length; j < l; j++) {
		const n = mod(round$1(lerp$1(fn[j], tn[j], progress), precision));
		const s = ts[j + 1];
		v += `${s ? n + s : n}`;
		tween._numbers[j] = n;
	}
			...
			while (tween) {
				...
				if ((forcedRender || (tweenCurrentTime !== tweenChangeDuration || absoluteTime <= tweenNextRepTakeover || tweenPrevRep && !tweenPrevRepIsCrossParent && (!tweenNextRep || tweenNextRep.parent !== tween.parent)) && (tweenCurrentTime !== 0 || absoluteTime >= tween._absoluteStartTime || tweenPrevRepIsCrossParent && !tween._hasFromValue && !tweenPrevRep._isOverridden && absoluteTime >= tweenPrevRepEndTime || tweenNextRep && !tweenNextRep._isOverridden && tweenNextRep.parent === tween.parent && tweenNextRep._currentTime !== 0 && iterationTime < tweenNextRep._startTime)) && (!tweenPrevRep || tweenPrevRepIsCrossParent || iterationTime >= tween._startTime) && (!tweenHasComposition || !tween._isOverridden && (!tween._isOverlapped || absoluteTime <= tweenAbsEndTime) && (!tweenNextRep || tweenNextRep._isOverridden || absoluteTime <= tweenNextRepTakeover) && (!tweenPrevRep || tweenPrevRep._isOverridden || (!tweenPrevRepIsCrossParent ? absoluteTime >= tweenPrevRepEndTime + tween._delay : absoluteTime >= tween._absoluteStartTime || !tween._hasFromValue && absoluteTime >= tweenPrevRepEndTime)))) {
					...
					if (tweenIsNumber) value = number = tweenModifier(round$1(lerp$1(tween._fromNumber, tween._toNumber, tweenProgress), tweenPrecision));
						...
						ns[3] = tweenModifier(lerp$1(fn[3], tn[3], tweenProgress));
						if (!tween._setter || internalRender) value = `rgba(${round$1(ns[0], 0)},${round$1(ns[1], 0)},${round$1(ns[2], 0)},${ns[3]})`;
...
var set = (targets, parameters) => {
	...
	if (globals.editor && globals.editor.addSet) return globals.editor.addSet(targets, parameters);
	parameters.duration = minValue;
	...
	*/
	constructor(parameters = {}) {
		...
		sync(() => {
			...
			if (!this.target) {
				const target = parseTargets(parameters.target)[0];
	...
	handleScroll() {
		...
		if (shouldSeek) {
			...
			if (syncSmooth && isNum(syncSmooth)) {
				if (syncSmooth < 1) {
					const snap = lp < p && p === 1 ? 1e-4 : lp > p && !p ? -1e-4 : 0;
...
var normalizeTweenValue = (propName, value, $el, i, parsedTargets) => {
	let v = isStr(value) ? value : getFunctionValue(value, $el, i, parsedTargets, null, null);
	if (!isNum(v)) return v;
...
var parseIndividualTweenValue = ($el, propName, from, to, i, parsedTargets) => {
	...
	if (!isUnd(from)) tweenValue = [normalizeTweenValue(propName, from, $el, i, parsedTargets), computedTo];
	else tweenValue = isArr(to) ? to.map((v) => normalizeTweenValue(propName, v, $el, i, parsedTargets)) : computedTo;
	return tweenValue;
};
var WAAPIAnimation = class {
	...
	*/
	constructor(targets, params) {
		...
		const parsedTargets = registerTargets(targets);
		if (!parsedTargets.length) console.warn(`No target found. Make sure the element you're trying to animate is accessible before creating your animation.`);
		...
		parsedTargets.forEach(($el, i) => {
			...
			/** @type {Number} */
			const duration = (spring ? spring.settlingDuration : getFunctionValue(setValue(params.duration, globals.defaults.duration), $el, i, parsedTargets, null, null)) * timeScale;
			/** @type {Number} */
			...
			for (let name in params) {
				...
				if (isObj(propertyValue)) {
					...
					/** @type {Number} */
					tweenParams.duration = (tweenOptionsSpring ? tweenOptionsSpring.settlingDuration : getFunctionValue(setValue(tweenOptions.duration, duration), $el, i, parsedTargets, null, null)) * timeScale;
					/** @type {Number} */
					...
					} else keyframes[name] = parseIndividualTweenValue($el, name, from, to, i, parsedTargets);
					...
					parsedPropertyValue = isArr(propertyValue) ? propertyValue.map((v) => normalizeTweenValue(name, v, $el, i, parsedTargets)) : normalizeTweenValue(name, propertyValue, $el, i, parsedTargets);
					if (individualTransformProperty) {
						keyframes[`--${individualTransformProperty}`] = parsedPropertyValue;
						cachedTransforms[individualTransformProperty] = parsedPropertyValue;
...
var updateNodeTimingParams = (node, params) => {
	...
	node.duration = hasSpring ? keyEasing.settlingDuration : getFunctionValue(params.duration, node.$el, node.index, node.targets, null, null);
	...
	*/
	animate(params = {}) {
		...
		tlParams.onComplete = () => {
			...
			if (ap && ap.linked || ed && ed.showPanel) {
...
* @return {FunctionValue}
*/
var morphTo = (path2, precision = .33) => ($path1, index, total, prevTween) => {
...
//#endregion
//#region node_modules/animejs/dist/modules/text/scramble.js
/**
* Anime.js - text - ESM
* @version v4.5.0
* @license MIT
...
var expandCharRanges = (str) => {
	let result = "";
	for (let i = 0, l = str.length; i < l; i++) if (i + 2 < l && str[i + 1] === "-" && str.charCodeAt(i) < str.charCodeAt(i + 2)) {
...
var scrambleText = (params = {}) => {
	...
	return (target, index, targets, prevTween) => {
		...
		const overrideChars = override === true ? characters : typeof override === "string" && override.length > 0 ? expandCharRanges(charSets[override] || override) : null;
		...
		if (endLength < animLength && resolvedFrom !== "left" && resolvedFrom !== "right" && resolvedFrom !== "random") {
			let maxExtraEnd = 0;
			for (let c = endLength; c < animLength; c++) if (charEnds[c] > maxExtraEnd) maxExtraEnd = charEnds[c];
			...
			for (let i = 0; i < endLength; i++) {
				...
				if (revealTime > charEnds[targets[i]]) charEnds[targets[i]] = revealTime;
			}
		}
		const charCache = new Array(animLength);
		for (let c = 0; c < animLength; c++) charCache[c] = characters[rng(0, totalChars)];
		const overrideCache = overrideChars ? overrideChars === characters ? charCache : new Array(animLength) : null;
		if (overrideCache && overrideCache !== charCache) for (let c = 0; c < animLength; c++) overrideCache[c] = overrideChar || overrideChars[rng(0, overrideChars.length - 1)];
		let fillStartText = startingText;
		if (!prevTween) {
			if (override === "") fillStartText = "";
			else if (overrideChars) {
				fillStartText = "";
				for (let c = 0; c < startLength; c++) fillStartText += startingText[c] === " " ? " " : 				/** @type {Array<String>} */ overrideCache[c];
			}
		}
		let lastValue = -1;
		let lastStep = -1;
		...
		return {
			...
			modifier: (v) => {
				...
				for (let c = 0; c < animLength; c++) {
					...
					if (t <= 0 || t < charStart) {
						if (hasOverride && c < startLength) if (hasOverrideChars) if (startingText[c] === " ") scrambled += " ";
						else {
							if (refreshChars) /** @type {Array<String>} */ overrideCache[c] = overrideChar || overrideChars[rng(0, totalOverrideChars)];
							scrambled += overrideCache[c];
						}
						else scrambled += startingText[c];
						continue;
					}
					if (c < endLength && settledText[c] === " " || c < startLength && startingText[c] === " ") scrambled += " ";
					else if (hasCursor && t - charStart < cursorZone) scrambled += cursorChars[cursorLen - 1 - ((t - charStart) / settleSpacing | 0)];
					else {
...
*/
//#endregion
//#region node_modules/animejs/dist/modules/index.js
/**
* Anime.js - ESM
* @version v4.5.0
* @license MIT
