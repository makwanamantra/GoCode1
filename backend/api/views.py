...
class CustomRequestViewSet(viewsets.ModelViewSet):
    queryset = CustomRequest.objects.select_related('client').all().order_by('-created_at')
    ...
    def perform_create(self, serializer):
        custom_req = serializer.save()
        AuditLog.objects.create(
            user=custom_req.client,
            action="Custom Request Submitted",
            details=f"Client submitted request '{custom_req.title}'"
        )
...
class ProjectViewSet(viewsets.ModelViewSet):
    ...
    def pay_advance(self, request, pk=None):
        ...
        if project.advance_paid:
            return Response(
                {'error': 'Advance already paid for this project.',
                 'project': ProjectSerializer(project).data},
                status=400,
            )
=======
...
<<<<<<< HEAD
    def get_queryset(self):
        ...
        employee = self.request.query_params.get('assigned_employee')
        status_filter = self.request.query_params.get('status')
        if project:
    ...
    def perform_create(self, serializer):
        ...
        AuditLog.objects.create(
            ...
            details=f"Task '{task.title}' assigned to "
...
<<<<<<< HEAD
    ...
    def perform_create(self, serializer):
        ...
        AuditLog.objects.create(
            user=deliverable.uploaded_by,
            action="Deliverable Submitted",
            details=f"'{deliverable.title}' submitted for project "
                    f"'{getattr(deliverable.project, 'title', 'N/A')}'"
        )


class ChatMessageViewSet(viewsets.ModelViewSet):
    """Threaded chat for a project or a custom request.

    Reads should always be scoped with `?project=<id>` or
    `?custom_request=<id>` so a thread only returns its own messages.
    """

    queryset = ChatMessage.objects.select_related('sender').all().order_by('timestamp')
    serializer_class = ChatMessageSerializer

    def get_queryset(self):
        qs = super().get_queryset()
        project = self.request.query_params.get('project')
        custom_request = self.request.query_params.get('custom_request')
        sender = self.request.query_params.get('sender')
        if project:
            qs = qs.filter(project_id=project)
        if custom_request:
            qs = qs.filter(custom_request_id=custom_request)
        if sender:
    ...
    def perform_create(self, serializer):
        ...
        name = (serializer.validated_data.get('sender_name') or '').strip()
        role = (serializer.validated_data.get('sender_role') or '').strip()
        if sender:
            if not name:
